...
import com.example.appBandas.servicios.institucion.BandaServicio;
...
public class BandaControlador {
    ...
    @GetMapping("/cif/{cif}")
    public ResponseEntity<Banda> obtenerPorCif(@PathVariable String cif) {
        Optional<Banda> banda = bandaServicio.obtenerBandaPorCif(cif);
        if (banda.isPresent()) {
            return ResponseEntity.ok(banda.get());
        } else {
            return ResponseEntity.notFound().build();
